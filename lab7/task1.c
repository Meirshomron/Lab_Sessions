...
void PrintHex(char* buffer, int length){
	int j;
	if (size==1) {
		for (j=0; j<length; j=j+size){
			char unity[17];
			sprintf(unity,"%02x", (unsigned char)buffer[j]);
			int toPrint = (int)strtol(unity,NULL,16);
			printf("%02x ",toPrint);
		}
		printf("\n");
		...
		for (j=0; j<length; j=j+size){
			char unity[17];
			sprintf(unity,"%02x%02x", (unsigned char)buffer[j+1],(unsigned char)buffer[j]);
			int toPrint = (int)strtol(unity,NULL,16);
			printf("%04x ",toPrint);
		}
		printf("\n");
		...
		for (j=0; j<length ; j=j+size){
			char unity[17];
			sprintf(unity,"%04x%04x%04x%04x", (unsigned char)buffer[j+3],(unsigned char)buffer[j+2],(unsigned char)buffer[j+1],(unsigned char)buffer[j]);
			int toPrint = (int)strtol(unity,NULL,16);
			printf("%08x ",toPrint);
		}
		printf("\n");
...
void badPrintHex(char* buffer, int length){
	int j,i;
	if (size==1){
		for (j=0; j<length; j=j+size){
			for (i=size-1; i>=0; i--)
		...
		for (j=0; j<length; j=j+size){
			for (i=size-1; i>=0; i--)
				printf("%04x ",(unsigned char) buffer[i+j]);	
		}	 
		printf("\n");
	}	
}
...
void PrintDec(char* buffer, int length){
	int j;
	if (size==1) {
		for (j=0; j<strlen(buffer)*size; j=j+size){
			int i;
			for (i = size -1 ; i>= 0; i--)
				printf("%d",(unsigned char)buffer[j+i]);
			printf(" ");
		}
		printf("\n");
	}
		...
		for (j=0; j<length ; j=j+size){
			char unity[17];
			sprintf(unity,"%02x%02x%02x%02x", (unsigned char)buffer[j+3],(unsigned char)buffer[j+2],(unsigned char)buffer[j+1],(unsigned char)buffer[j]);
			int toPrint = (int)strtol(unity,NULL,16);
			printf("%d ",toPrint);
		}
		printf("\n");
...
int sourceAdd;
	...
	printf("Debug: the fileSize is ! %d\n",fileSize);
  	...
  	int ret=write(fd,(void *)sourceAdd,newLength); /* 101 */
