...
void printSecNames(){
   ...
   const char * const sh_strtab_p = map_start + sh_strtab->sh_offset;/* sh_strtab_p points to the string table of section headeras*/
   printf("[Nr]	Name		Addr		Off	Size	Type\n");
   for (i=0;i< header->e_shnum; i++){ /* e_shnum is the number of section headers in the table */
   		printf("[%2d]	%-10s",i, sh_strtab_p + sections[i].sh_name); /*the sections[i].sh_name is the index of the symbol's name  */			
    	...
    	if (sections[i].sh_type==SHT_SYMTAB) {
        	symtab = (Elf32_Sym *)((char *)map_start + sections[i].sh_offset);
       		int entries=sections[i].sh_size/(sizeof(Elf32_Sym));
       		 ...
       		 int off=0;
       		 int placeOfStrTab;
       		 printf("\nNum: Value  Ndx Section_Name Symbol_Name\n");
    	 	/*printf("places1: %d %d\n",placeOfStrTab, sections[i].sh_link);	*/
		     Elf32_Shdr* strtab = &sections[sections[i].sh_link]; /* shstrndx is the index of the string table of the section header's names */
        	...
        	symtab = (Elf32_Sym *)((char *)map_start + sections[i].sh_offset);
 		     ...
 		     Elf32_Shdr* dyn_strtab = &sections[sections[i].sh_link];  
     		 off = dyn_strtab->sh_offset;
...
void linkCheck(){
   ...
   for (i = 0; i < header->e_shnum; i++){
    	if (sections[i].sh_type==SHT_SYMTAB) {
        	symtab = (Elf32_Sym *)((char *)map_start + sections[i].sh_offset);
       		int entries=sections[i].sh_size/(sizeof(Elf32_Sym));/*number of symbols in the symbol table*/
       		 int j;
       		 int off=0;
    		 int placeOfStrTab = sections[i].sh_link;
		     Elf32_Shdr* strtab = &sections[placeOfStrTab]; /*pointer to the section of the string table of the symbol's names*/
		     off = strtab->sh_offset;
		     const char * const strtab_p = map_start + off; /*pointer to the start of the string table*/
       		 for(j=0;j<entries;j++){
...
int printIndexAtSymTab(int index, int locationOfSymbolTable){
   int has_name2=0;
   int has_name1=0;
   Elf32_Shdr* sections = (Elf32_Shdr *)((char *)map_start + header->e_shoff); /* shoff is the offset of the section header table of the file */
   Elf32_Sym* symTab;
   int i;
        	 symTab = (Elf32_Sym *)((char *)map_start + sections[locationOfSymbolTable].sh_offset);
        	 int j;
    		 int placeOfStrTab = sections[locationOfSymbolTable].sh_link;
		     Elf32_Shdr* strTab = &sections[placeOfStrTab]; /* shstrndx is the index of the string table of the section header's names */
		     int off = strTab->sh_offset;
 		     const char * const strTab_p = map_start + off;
       		 has_name2=printf("%s ", strTab_p + symTab[index].st_name);          
    
    if(has_name2<=1&&has_name1<=1)
    	return 0;
    return 1;
}
	...
	for (i = 0; i < header->e_shnum; i++){
    	if (sections[i].sh_type==SHT_REL || sections[i].sh_type==SHT_RELA) {
        	...
        	int entries=sections[i].sh_size/(sizeof(Elf32_Rel));
        	 int locationOfSymbolTable = sections[i].sh_link; 
       		 int j;
       		 int off=0;
        		...
        		printType(tmpType);
        		int printed_name=printIndexAtSymTab(strOffset,locationOfSymbolTable);
        		printf("\n");
       		 }
       		 printf("\n");
        }
